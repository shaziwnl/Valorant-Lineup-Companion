const videoLinks: any = {
  // Make sure to have names with the following format
  // Agentname Map Utility PlaceOnMap Attack/Defence 
  // eg: Kayo Ascent Flashdrive ASite Attack
  // eg: Kayo Ascent Flashdrive ASite Heaven Attack
  // we only want to keep - "A Site Heaven Attack" or "A Site Attack"
  Ascent: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [{title: "Kayo Ascent Flashdrive B Site Lane Attack", id: 'ixsyw_XWQr0'},
            {title: "Kayo Ascent Flashdrive A Site A Main Attack", id: 'UorJp6lLH-Q'},
            {title: "Kayo Ascent Flashdrive A Site Attack", id: 'ixsyw_XWQr0'},
            {title: "Kayo Ascent Flashdrive A Site A Main Defense 1", id: 'UorJp6lLH-Q'},
            {title: "Kayo Ascent Flashdrive A Site A Main Defense 2", id: 'ixsyw_XWQr0'},
            {title: "Kayo Ascent Flashdrive B Site Boat + Logs Attack", id: 'UorJp6lLH-Q'},
            {title: "Kayo Ascent Flashdrive B Site Post Plant Boat Attack", id: 'ixsyw_XWQr0'},
            {title: "Kayo Ascent Flashdrive Middle Market Attack", id: 'UorJp6lLH-Q'}],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Bind: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Lotus: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Pearl: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Haven: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Icebox: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Breeze: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Fracture: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },

  Split: {
    Astra : [],
    Brimstone : [],
    Clove : [],
    Iso : [],
    Jett : [],
    Omen : [],
    Viper : [],
    Yoru : [],
    Sage : [],
    Skye : [],
    Sova : [],
    Breach : [],
    Deadlock : [],
    Chamber : [],
    Gekko : [],
    Kayo : [],
    Neon : [],
    Pheonix : [],
    Raze : [],
    Reyna : [],
    Fade : [],
    Cypher : [],
    Killjoy : [],
  },
}

export default videoLinks;